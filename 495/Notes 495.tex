...
\subsection{EM PPCA}
\begin{enumerate}
\item Write out the log likelihood of the joint distribution of the observed variable and the latent variable
\begin{align*}
...
\item Take the expectation on the log likelihood on the joint distribution:
\begin{align*}
\ln p(\vec{X,Y} \vert \vec{\theta}) 
&=\sum_{i=1}^N\left[-\frac{F}{2}\ln 2\pi\sigma^2 -\frac{1}{2\sigma^2}\vec{(x_i-Wy_i-\mu)^\top (x_i-Wy_i-\mu)} -\frac{1}{2}\vec{y_i^\top y_i}-\frac{D}{2}\ln 2\pi\right]\\
