...
\begin{align*}
	\vec{x_i} &= \vec{\mu_k} + \vec{W_ky_{ik}} + \vec{e_{ik}}\\
	\vec{y_{ik}} &\sim \mathcal{N} (\vec{y_{ik}}\vert \vec{0,I})\\
	\vec{e_{ik}} & \sim \mathcal{N} (\vec{e_{ik}}\vert \vec{0},\sigma^2_k\vec{I}), \forall k = 1,\dots, K\\
	p(\vec{x_i}\vert  \vec{z_{ik}} &=1, \vec{y_{ik}}, \vec{\mu_k}, \vec{W_k}, \sigma^2_k \vec{I}) = \mathcal{N}(\vec{x_i} \vert \vec{\mu_k} + \vec{W_ky_{ik}})\\
	p(\vec{x_i}\vert \vec{\theta})& = \sum_{k=1}^K \pi_k \mathcal{N}(\vec{x_i} \vert \vec{\mu}_k, \vec{D}_k)
\end{align*}
...
\item Priors:
	\begin{align*}
		p(\vec{z_i} \vert \vec{\theta_z}) & = \prod_{k=1}^K \pi_k^{z_{ik}}\\
		p(\vec{Y_i} \vert \vec{z_i}, \vec{\theta_z}) & = \prod_{k=1}^K p(y_{ik})^{z_{ik}} = \prod_{k=1}^K \mathcal{N}(y_{ik}\vert \vec{0}, \vec{I})^{z_{ik}}
	\end{align*}
