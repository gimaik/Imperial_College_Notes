...
Similar to the HMM, we denote the following:
\begin{align*}
...
We develop the same recursive formula for the $\beta$:
\begin{align*}
...
By going through the same algebra manipulation as $\alpha$
\begin{align*}
	\gamma(\vec{z}_n) 	&= \hat{\alpha}(\vec{z}_n)\hat{\beta}(\vec{z}_n) = \mathcal{N}(\vec{z}_n\vert \hat{\vec{\mu}}_n, \hat{\vec{V}}_n)\\
...
The smoothed transition probability is given by:
\begin{align*}
