...
\begin{enumerate}
	...
	\item For each clique, $cli$, find the set of its variables $\lbrace X_i \rbrace$ whose parents $Pa(X_i)$ also belongs to the clique and initialize the clique potential function as follows:
	\begin{align*}
		\psi(cli) = \psi(W_i) = \prod_{\lbrace X_i \rbrace} P(X\vert Pa(X))
	\end{align*}
\end{enumerate}
